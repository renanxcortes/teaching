...
![Resumo geral do conjunto de dados (introduce / plot_intro)](figs/credito_charts/01_plot_intro.png)
...
![Verificação de valores ausentes (plot_missing)](figs/credito_charts/02_plot_missing.png)

Confirmando o comentário do script: `OCCUPATION_TYPE` é, de fato, a única coluna com valores ausentes na base combinada (cerca de 30%) — um exemplo prático do tipo de inconsistência discutido na página de [Desafios de Bases Alinhadas ao Negócio](4_desafios_base_negocio.qmd).
...
![Distribuição da variável resposta (plot_bar)](figs/credito_charts/03_plot_bar_y.png){width="50%"}
...
![Distribuição das variáveis numéricas (plot_histogram)](figs/credito_charts/04_plot_histogram.png)
...
![Distribuição das variáveis categóricas — top 10 classes por variável (plot_bar)](figs/credito_charts/05_plot_bar_categorical.png)
...
![Correlação entre variáveis numéricas (plot_correlation)](figs/credito_charts/06_plot_correlation.png){width="70%"}
...
![Comparação das variáveis numéricas por classe (plot_boxplot, by = "y")](figs/credito_charts/07_plot_boxplot_by_y.png)
